-- Migration: Initial schema for web scraping feature
-- Description: Creates scrapes and scraped_pages tables with RLS policies
...
DROP POLICY IF EXISTS "Allow read access to all scrapes" ON scrapes;
DROP POLICY IF EXISTS "Allow insert for authenticated users" ON scrapes;
DROP POLICY IF EXISTS "Allow update own scrapes" ON scrapes;
DROP POLICY IF EXISTS "Allow delete own scrapes" ON scrapes;
...
-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Allow read access to all scraped pages" ON scraped_pages;
DROP POLICY IF EXISTS "Allow insert scraped pages for own scrapes" ON scraped_pages;
DROP POLICY IF EXISTS "Allow update scraped pages for own scrapes" ON scraped_pages;
DROP POLICY IF EXISTS "Allow delete scraped pages for own scrapes" ON scraped_pages;
